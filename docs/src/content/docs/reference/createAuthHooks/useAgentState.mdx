...
| `isLocalhost`    | `boolean`             | Whether the agent is connected to a development network: `network` is `"local"` or `"remote"`                                                                                   |
| `network`        | `string \| undefined` | The network the agent is connected to — `"ic"`, `"local"`, or `"remote"` (a replica forwarded by Codespaces or Gitpod; a page on those domains gets it without a `host` option) |
