...
These are the only two members of `ClientManagerParameters`. Network detection needs no flag — see [Network Configuration](#network-configuration).
...
When your app is served by the `@icp-sdk` toolchain (e.g., `icp-cli`) or by [`@ic-reactor/vite-plugin`](/v3/packages/vite-plugin), the dev server injects an `ic_env` cookie carrying the local canister IDs and root key. `ClientManager` reads that cookie **automatically** in the browser — there is no option to enable:
...
- Root key from the cookie, accepted only on non-mainnet hosts
