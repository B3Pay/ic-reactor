...
Run an imperative fetch of a registered canister's query, and run it again when the principal switches while it runs:
...
`updateAgent()` cancels those queries when the principal changes. A query with a mounted observer refetches afterwards, but an imperative fetch has no observer. TanStack rejects its promise with a `CancelledError`, or, when the entry already held data before the fetch began (a stale entry that `fetchQuery` or `fetchInfiniteQuery` refetches), resolves it with that data, which is the previous principal's. Through this method it resolves with the answer for the identity installed now. `Reactor.fetchQuery()`, the query factories' `fetch()` and `prefetch()`, and a query method's `call()` and `refetch()` from `useActorMethod` already run through it.

Whatever the fetch settles with while the principal stays is passed on as is, including a cancellation from your own `cancelQueries()`. When it has run again three times and another switch overtook each of those runs too, it rejects with a `CallError` rather than return an answer that may belong to a previous principal. `fetch` is called once per run, so it must start a new fetch each time.
...
- **In-flight queries are cancelled**, so a response signed by the previous identity cannot land after the switch. A `fetchQuery()` or factory `fetch()` that was waiting on one runs again for the new identity and resolves with that answer, as does a query method's `call()` or `refetch()` from `useActorMethod`, and a factory `prefetch()` runs again too (see [fetchAcrossIdentitySwitch](#fetchacrossidentityswitch))
...
A new identity for the principal already installed is not a switch: the cached answers are already that principal's, so the sweep is skipped. Only the entries whose last fetch failed are invalidated, since the previous identity (an expired delegation, say) may be why they failed. A fetch already in flight, signed by the previous identity, is left to finish, and its answer is kept; if it fails instead, its entry is invalidated too. The first `updateAgent()` after construction always sweeps, whatever it installs.
