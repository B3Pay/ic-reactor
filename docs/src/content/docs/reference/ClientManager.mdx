...
<Aside type="note">
  ...
  has the agent fetch the root key before its first request. Any other host
  counts as mainnet and never fetches it. `initialize()` fetches it up front and
  sets `agentState.isInitialized`, for a loading screen that waits on it. The
