...
For non-mainnet hosts, `ClientManager` also uses the cookie-provided `IC_ROOT_KEY`
when present. Treat this as local-development convenience only: browser cookies
are user-controllable, and mainnet hosts do not consume cookie-provided root
keys. If you need an explicit custom local replica root key, pass
