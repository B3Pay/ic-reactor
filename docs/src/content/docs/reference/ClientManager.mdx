...
- Agent host taken from `window.location.origin` when the page is served from a local or IC boundary host. In a web worker, which has no `window`, it is taken from the worker's `location.origin`, the origin of the page that started it
...
<Aside type="tip">
  ...
  (or an IC boundary domain), and a web worker infers it from its own
  `location.origin` the same way. In Node/SSR, `ICP_NETWORK`/`DFX_NETWORK` is
  read from `process.env`. Pass `agentOptions.host` to override either.
</Aside>
