...
Whatever the fetch settles with while the principal stays is passed on as is, including a cancellation from your own `cancelQueries()`. After three runs in a row, each overtaken by another switch, it rejects with a `CallError` rather than return an answer that may belong to a previous principal. `fetch` is called once per run, so it must start a new fetch each time.
