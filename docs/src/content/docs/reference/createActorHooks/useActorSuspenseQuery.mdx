...
<Aside type="note">
  `args` does not take `skipToken`: a suspense query has to be able to run.
  Mount the component only once its arguments exist, or use
  [useActorQuery](/v3/reference/createactorhooks/useactorquery#waiting-for-args),
  which does.
</Aside>
