...
| `getArgs`          | `((pageParam) => Args) \| skipToken` | Function that returns args for each page, or `skipToken` while they are not known |
...
| `getKeyArgs`           | `function` | -        | Narrows the args that key the cache (see below) |

The cache key includes the first page's arguments, `getArgs(initialPageParam)`,
so two lists over the same method with different arguments stay apart. When
those arguments carry the cursor, `getKeyArgs` returns the part that identifies
the list. It is required when `initialPageParam` changes between renders, as
`Date.now()` does: without it every render keys a new query and refetches the
first page.

While the list's arguments are not known yet, pass TanStack Query's
`skipToken` (re-exported by `@ic-reactor/react`) as `getArgs`. The list waits
without calling the canister, in a cache entry of its own under its method and
`queryKey`, and fetches its first page once `getArgs` is a function:
...
const { data } = useActorInfiniteQuery({
  ...
  getArgs: authorId
    ? (offset: number) => [{ authorId, offset, limit: 10 }]
    : skipToken,
  initialPageParam: 0,
...
`useActorSuspenseInfiniteQuery` does not take `skipToken`.
...
const { data, fetchNextPage, fetchPreviousPage, hasNextPage, hasPreviousPage } =
  useActorInfiniteQuery({
    ...
    // `Date.now()` differs on every render, so keep the cursor out of the key.
    getKeyArgs: ([{ limit }]) => [{ limit }],
...
- [useActorSuspenseInfiniteQuery](/v3/reference/createactorhooks/useactorsuspenseinfinitequery) — Suspense version
- [useActorQuery](/v3/reference/createactorhooks/useactorquery) — Regular queries
