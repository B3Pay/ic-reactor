...
| `invalidateQueries` | `InvalidationTarget[]`          | -       | Queries to invalidate on success, before `onSuccess`                  |
| `onCanisterError`   | `function`                      | -       | Called only for `Result { Err }` variants (canister logic errors)     |
| `retry`             | `number \| boolean \| function` | `0`     | Number of retry attempts, or a predicate such as `reactorUpdateRetry` |
...
```tsx
import { getPost } from "../queries" // createQueryFactory(backend, ...)
...
const { mutate } = useActorMutation({
  ...
  invalidateQueries: [
    ...
    getPost, // every query this factory returned
  ],
})
```

Each entry is a `{ functionName, args? }` method of the reactor, type-checked
against the canister, a query object or query factory, or a query key. The
invalidation is awaited before `onSuccess` runs, so the mutation stays pending
until the invalidated queries in use have refetched. `undefined` entries are
skipped.
...
function LikeButton({ postId }: { postId: string }) {
  const { mutate } = useActorMutation({
    ...
    // Cancel outgoing refetches, write the new value, keep a rollback
    onMutate: ([postId]) =>
...
The updater is typed from the query and gets its cached raw value; it is not
called when nothing is cached. See
[Optimistic Updates](/v3/framework/mutations#optimistic-updates).
...
<Aside type="tip">
  Use `invalidateQueries` to refresh related data. For cache writes, such as
  putting a created item into a cached list, use a query object's `setData()` in
  `onSuccess`.
</Aside>
...
<Aside type="note">
  Unlike queries, mutations don't automatically retry by default. A retry of an
  update method is a new call the canister runs, so `retry: 3` can execute a
  transfer twice when a response is lost. `retry: reactorUpdateRetry` retries
  only a `SysTransient` rejection, which proves the call never ran. A
  `mutations.retry` in the QueryClient's defaults applies to every mutation,
  this hook's included.
</Aside>

## See Also

- [Mutations Guide](/v3/framework/mutations) — In-depth guide to mutations
- [useActorQuery](/v3/reference/createactorhooks/useactorquery) — Query hook reference
- [Query Caching](/v3/framework/query-caching) — Cache invalidation patterns
