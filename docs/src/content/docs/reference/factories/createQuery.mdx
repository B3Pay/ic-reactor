...
The prefix is the reactor's canister and the method, so it also matches a
query of the same method made elsewhere, such as with `useActorQuery`. A
config `queryKey` comes after the args in each instance's key, so it does not
narrow the prefix.
