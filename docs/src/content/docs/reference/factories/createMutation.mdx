...
follows a `setCanisterId`. An infinite list keys its pages apart from its
args, so name one by its query object or factory, or by the method alone. A
query object or factory is invalidated in its own reactor's QueryClient.
