...
Each level's `onSuccess`, `onError` and `onSettled` receive the return value of
that level's own `onMutate` as their third argument. Factory callbacks get the
factory's result and hook callbacks get the hook's, so rollback state can live
at either level. A factory without its own `onMutate` hands its callbacks the
hook's result.
