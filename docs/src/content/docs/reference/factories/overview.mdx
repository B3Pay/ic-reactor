...
<Aside type="tip">
  With the [CLI](/v3/packages/cli) or the [Vite
  plugin](/v3/packages/vite-plugin), set `factories: true` on a canister to
  generate a `createQuery`, `createQueryFactory` or `createMutation` object for
  every method of its `.did`, chosen by the method's annotation. See [Query and
  Mutation Factories](/v3/packages/codegen#query-and-mutation-factories).
</Aside>
...
<CardGrid>
  <LinkCard
    ...
    href="/v3/reference/factories/createquery"
  />
  <LinkCard
    title="createSuspenseQuery"
    description="Suspense-enabled query factory"
    href="/v3/reference/factories/createsuspensequery"
  />
</CardGrid>
...
<CardGrid>
  <LinkCard
    ...
    href="/v3/reference/factories/createmutation"
  />
</CardGrid>
...
<CardGrid>
  <LinkCard
    ...
    href="/v3/reference/factories/createinfinitequery"
  />
  <LinkCard
    title="createSuspenseInfiniteQuery"
    description="Suspense-enabled infinite queries"
    href="/v3/reference/factories/createsuspenseinfinitequery"
  />
</CardGrid>
...
export const createPostMutation = createMutation(backend, {
  ...
  // A query factory covers every query it returns, whatever the args
  invalidateQueries: [getPostsQuery],
})

// Query keys, for QueryClient calls of your own
export const queryKeys = {
  users: () => getUserQuery.getQueryKey(), // every getUser query
...
<Aside type="tip">
  `createQueryFactory` and `createSuspenseQueryFactory` take the **args tuple**,
  not a bare value — the same array you would pass to `args`. Their results are
  cached, so calling `getUserQuery(["alice"])` again returns the same object
  instance. `createInfiniteQueryFactory` and
  `createSuspenseInfiniteQueryFactory` take a `getArgs` function instead, and
  build a new query object on every call.
</Aside>
...
- [Reactor](/v3/reference/reactor) — Understanding query keys
