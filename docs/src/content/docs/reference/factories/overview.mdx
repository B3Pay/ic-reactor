...
<Aside type="tip">
  A `*Factory` variant takes the **args tuple**, not a bare value — the same
  array you would pass to `args`. Results are cached internally, so calling
  `getUserQuery(["alice"])` multiple times returns the same object instance.
</Aside>
