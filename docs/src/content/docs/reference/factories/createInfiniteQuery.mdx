...
IC Reactor-specific options below. `networkMode`, `retry`, `retryDelay` and
`meta` also apply to `fetch()`; the rest apply to the hook only.
...
`invalidate()`, or `reset()` to drop the loaded pages and start again from the
first one.

`optimisticUpdate` gets and returns the raw `InfiniteData`,
`{ pages, pageParams }`, before `select`. For example, to show a new post at
the top of the first page while it is being created:

```typescript
const draft = await postsQuery.optimisticUpdate((data) => ({
  ...data,
  pages: [[newPost, ...data.pages[0]], ...data.pages.slice(1)],
}))
// in onError: draft.rollback()
```
...
list it returns, whatever its `getArgs` and however many pages it loaded:

```typescript
// Refetch every loaded list
await getPostsQuery.invalidate()

// Or after a mutation
const createPost = createMutation(backend, {
  functionName: "createPost",
  invalidateQueries: [getPostsQuery],
})
```

The prefix is built from the method, the factory's `callConfig` and its config
`queryKey`, the part of the key every instance shares.
...
<Aside type="caution">
  ...
  [createSuspenseInfiniteQuery](/v3/reference/factories/createsuspenseinfinitequery)
  instead.
</Aside>

## See Also

- [createSuspenseInfiniteQuery](/v3/reference/factories/createsuspenseinfinitequery) — Suspense version
- [useActorInfiniteQuery](/v3/reference/createactorhooks/useactorinfinitequery) — Direct hook usage
- [createQuery](/v3/reference/factories/createquery) — Non-paginated queries
