...
<Aside type="caution" title="Changed behavior — was size-dependent">
  In 3.10 and earlier, only blobs of 512 bytes or fewer decoded to hex;
  anything larger stayed a `Uint8Array`, so one field could change JS type
  with payload size and JSON-serialising a result silently corrupted large
  blobs. If you branched on `typeof blob === "string"`, the `Uint8Array`
