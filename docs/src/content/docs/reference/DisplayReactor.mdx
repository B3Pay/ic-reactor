...
### With defineDisplayReactor

`defineDisplayReactor` does the whole setup in one call — `QueryClient`,
`ClientManager`, the `DisplayReactor`, its hooks and the Internet Identity
hooks — and takes the same options as `defineReactor`, plus `validators`:
...
It replaces `defineReactor({ display: true })`, which still works but is
deprecated: the flag makes every `defineReactor` bundle carry `DisplayReactor`
and zod, whether or not it is set.

<Aside type="note">
  The display codecs are built on zod, which adds about 85 kB minified (24 kB
  gzipped) to a bundle that does not already include it. See [Bundle
  Size](/v3/packages/react#bundle-size) for what each setup path costs.
</Aside>
...
<CardGrid>
  ...
  <Card title="Optionals" icon="approve-check">
    `[T] | []` → `T | undefined` JavaScript-native nullish
  </Card>
...
| `float32` | `number`   | `number` | `NaN`, `±Infinity`, `-0` kept; [JSON loses them](#floats) |
| `float64` | `number`   | `number` | `NaN`, `±Infinity`, `-0` kept; [JSON loses them](#floats) |
...
| `vec (text, T)`  | `[string, T][]` | `{ [key: string]: T }` | [One value per key](#working-with-key-value-pairs) |
...
### Many Canisters of One Interface

Like `Reactor`, `DisplayReactor` gives a sibling for another canister of the
same interface with [`forCanister`](/v3/reference/reactor#forcanister). This is
the multi-token wallet pattern: one reactor per token, all on the same agent
and cache, with the display transform and validators passed on:

```typescript
import { DisplayReactor } from "@ic-reactor/core"
import { idlFactory, type _SERVICE } from "./declarations/icrc1"

// The reactor the siblings are made from. It still needs a canister ID of its
// own: the constructor throws without one outside a local replica.
...
async function getTokenInfo(canisterId: string) {
  // The same reactor each time for the same canister
  const token = tokenReactor.forCanister(canisterId)

  // Two calls for two tokens can run at once without crossing
  const [name, symbol, decimals] = await Promise.all([
    token.callMethod({ functionName: "icrc1_name" }),
    token.callMethod({ functionName: "icrc1_symbol" }),
    token.callMethod({ functionName: "icrc1_decimals" }),
  ])

  // name and symbol are text; decimals, a nat8, is a number
  return { name, symbol, decimals }
}
```

`setCanisterId` still retargets one reactor in place, for code that follows a
single selection, but every call and view built on that reactor moves with it.
...
function Balance({ principal }: { principal: string }) {
  ...
  // balance is the nat as text, in e8s. Format it exactly; Number(balance) / 1e8
  // loses digits.
  if (balance === undefined) return <span>…</span>
  return <span>{formatTokenAmount(balance, 8)} ICP</span>
}
```

See [Token amounts](/v3/reference/utilities#token-amounts) for
`formatTokenAmount` and `parseTokenAmount`.
...
const avatar = user.avatar // Already undefined or value
```

### Handling Blobs (Binary Data)

Every blob (`vec nat8`) decodes to a lowercase hex string with no `0x`
prefix, whatever its size. One Candid type, one JavaScript type — hex text is
always JSON-safe, so persisting or serialising a display value never corrupts
binary fields.
...
`float32` and `float64` display as JavaScript numbers, and every value a
canister can return, `NaN`, `Infinity`, `-Infinity` and `-0` included, is sent
back as the same bytes. As an argument a float is also taken as the text a form
holds, such as `"3.14"`, which must spell a finite number: `""`, `"abc"`,
`"Infinity"` and `"NaN"` are refused, as the form schemas of `@ic-reactor/candid`
refuse them. A finite number that `float32` cannot hold, such as `3.5e38`, is
refused too, since Candid encoding would send it as `Infinity`.

<Aside type="caution" title="JSON cannot hold every float">
  `JSON.stringify` writes `NaN`, `Infinity` and `-Infinity` as `null`, and `-0`
  as `0`. After a JSON round trip, such as a persisted query cache, SSR
  hydration or a value posted to your server, an `opt float64` that held `NaN`
  comes back as `null` and is sent as none, a required float comes back as
  `null` and the call is refused, and `-0` is sent as `0`. Where such values can
  occur (a rate computed as 0/0, for example), keep them out of JSON or
  serialise them yourself. Query keys are not affected: they write each of these
  values apart from every other.
</Aside>
...
As an argument, the map is also taken as the array of pairs or as a `Map`.
Each is sent as its entries in its own order: an object's, the array's, or the
`Map`'s insertion order. The argument's TypeScript type names only the object,
so the other two need a cast.

<Aside type="caution" title="An object holds each key once, in its own order">
  A JavaScript object cannot hold a key twice, and it puts integer-like keys
  (`"0"` to `"4294967294"`) first, in ascending order, before every other key.
  So when a result's `vec record { text; T }` becomes an object, a repeated key
  keeps only its last value (an HTTP response's second `Set-Cookie` header
  replaces the first), and integer-like keys move to the front: `10, 2, b, 1`
  displays as `1, 2, 10, b`. Sending that object back sends the vector it
  holds, not the one read. Where repeated keys or their order matter, such as
  HTTP headers, ICRC-21 consent message fields or ICRC-3 maps with numeric
  keys, call the method through a plain [`Reactor`](/v3/reference/reactor),
  which returns the array of pairs as received. A `DisplayReactor` argument
  given as the array of pairs, with a cast, is sent as it is.
</Aside>
...
This amount field takes base units (e8s). A field a person types a token
amount into, such as `"1.5"`, is read with
[`parseTokenAmount`](/v3/reference/utilities#parsetokenamount):
`mutate([recipient, parseTokenAmount(amount, decimals).toString()])`. It throws
on text it cannot read exactly instead of rounding it.
...
// data.bio is string | null | undefined (was [string] | []); none arrives as undefined
```

The display types come from the service type parameter
(`new DisplayReactor<_SERVICE>(...)`). Without one, the reactor is typed by
`BaseActor` and knows nothing about the Candid, so every method's display-side
arguments and results are `unknown`: validators receive `unknown` args, and
results must be narrowed before they are rendered. (An untyped `Reactor` types
the same values as `any`, because the raw side passes `BaseActor`'s `any`
through.) Pass the service type from your generated declarations, or cast to
the display shape you expect where the interface is only known at run time, as
with a [`CandidDisplayReactor`](/v3/packages/candid/candiddisplayreactor#types-with-and-without-a-service-type).
...
The schema checks the method's first argument. zod refuses a schema with an
async refinement or transform on its synchronous path, so pass
`{ async: true }` for one: the validator then parses with `safeParseAsync` and
returns a promise, and runs where async validators run (below).

```typescript
const recipientSchema = z.object({
  to: z
    .string()
    .refine(async (to) => !(await isBlocked(to)), "Address is blocked"),
})

backend.registerValidator(
  "transfer",
...
For async validation (e.g., checking a blocklist), use `validate()` or
...
`callMethod()` runs validators synchronously, and so do the query and mutation
hooks and factories that call it. They refuse a call whose validator returns a
promise, with a `CallError` that names the method, without sending anything.

### When a Validator Throws

A validator that throws or rejects (a blocklist lookup that fails, say) is
reported the same way by `callMethod()`, `callMethodWithValidation()` and
`validate()`: as a `CallError` whose `cause` is what the validator threw, and
the canister is not called. It is not a verdict on the arguments, so it is not
a `ValidationError`, and it is not the canister's answer, so a `CanisterError`
the validator's own lookup threw is not passed on as one. A `ValidationError`
the validator throws is passed on as it is.
...
try {
  ...
  if (isValidationError(error)) {
    // the validator's verdict: error.issues
  } else if (isCallError(error)) {
    // a failed call, or a validator that threw: error.cause
  }
}
```
...
- Serializing data to JSON (mind [non-finite floats](#floats))
...
<Aside type="caution">
  ...
  `BigInt(balance)`. To show a token amount or read one a person typed, use
  `formatTokenAmount` and `parseTokenAmount`, never `Number`.
</Aside>
...
- [createActorHooks](/v3/reference/createactorhooks/overview) — Create React hooks from a reactor
- [React Validation](/v3/reference/reactvalidation) — React validation utilities
- [Utilities](/v3/reference/utilities) — Exact token-amount formatting and parsing
- [Error Handling](/v3/guides/error-handling) — Handle validation and canister errors
