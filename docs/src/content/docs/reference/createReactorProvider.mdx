...
- **Unmount.** The provider calls `dispose()` on every `AuthenticationManager`
  the value holds, which releases the Internet Identity client it built: the
  value itself when it is one, those among its own properties, and the one each
  `defineReactor` result there uses. A `defineReactor` result whose tree never
  used authentication has built no manager, and none is built to be disposed.
...
<Aside type="caution">
  Everything the value holds belongs to the provider and is released with it.
  Build the managers inside the factory. A manager imported from module scope
  and returned by the factory is disposed whenever a provider holding it
  unmounts, while other trees may still use it.
</Aside>
