...
2. ## Configure Vite Plugin
   ...
   <Aside type="tip">
     When you start your dev server, the plugin creates
     `src/declarations/backend/` containing `index.generated.ts` (the managed
     reactor and hooks), a stable `index.ts` wrapper that re-exports it, and a
     `declarations/` folder with `backend.js`, `backend.d.ts`, and a copy of
     `backend.did`.
   </Aside>
...
In development the plugin injects local canister IDs through the `ic_env`
...
<Aside type="caution">
  IC Reactor never reads `VITE_*` variables itself. If you prefer to keep IDs in
  `.env`, you have to pass the value into a hand-written reactor yourself — see
