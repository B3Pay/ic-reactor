...
<Aside type="note">
  ...
  the agent host is local or a dev-container tunnel (a trusted `ic_env` cookie
  can supply it instead), so calls to a local replica work without an explicit
  `initialize()`. Any other host counts as mainnet and never fetches it. Calling
  `clientManager.initialize()` (as `useAuth` does) fetches it up front and sets
  `agentState.isInitialized`, which the example below waits for.
</Aside>
