...
| --------------------- | ------------------ | -------------------------------------------------------------------------------------- |
| **@ic-reactor/core**  | **~34 KB**         | Reactor, caching, transformations, validation (zod included)                           |
| **@ic-reactor/react** | **~12 KB**         | React hooks and the auth manager (excludes core, React, TanStack Query, @icp-sdk/auth) |

That's it. **~46 KB total** for a complete, production-ready data layer.

<Aside type="note">
  These are the numbers `pnpm size` reports (`size-limit`, gzipped), measured
  with peer dependencies excluded. The repository enforces budgets of 50 KB for
  `@ic-reactor/core` and 30 KB for `@ic-reactor/react`, so a regression fails
  CI.
</Aside>
...
<Aside>
  We leverage `@icp-sdk/core` as a peer dependency — you're already using it for
...
<CardGrid>
  <Card title="🧱 Single Responsibility" icon="seti:json">
...
| --------------------------- | -------------------------------- | -------------------------- |
| **Additional Bundle Size**  | **~46 KB** (on top of SDK)       | **~50+ KB** of custom code |
...
<Aside>
  ...
  layer in ~46 KB; building it yourself typically results in more code that does
  less.
</Aside>
...
<CardGrid>
  <Card title="🧠 AI-Friendly Codebase" icon="star">
  ...
  <Card title="📄 /llms.txt Standard" icon="document">
    We include `/llms.txt` in our documentation for structured API overviews
    that AI models can parse effectively.
  </Card>
  <Card title="🔮 Predictable Patterns" icon="approve-check">
    Every hook, every class follows the same patterns. Once you learn one, you
    know them all — and so does your AI pair programmer.
  </Card>
  <Card title="📚 Self-Documenting Types" icon="information">
...
| **Additional Code**       | ~46 KB (battle-tested)             | ~50+ KB (untested custom code) |
