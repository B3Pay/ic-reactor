...
IC Reactor is compatible with React 18+ and needs TypeScript 5.7 or later: its
declarations, and those of `@icp-sdk/core`, use generic typed arrays such as
`Uint8Array<ArrayBuffer>`, which TypeScript 5.7 introduced.
`@ic-reactor/react` needs `@tanstack/react-query` 5.90.2 or later.
...
against mainnet's. When you passed `agentOptions.rootKey` to `ClientManager`,
that agent gets your key instead, as your app's agent keeps it. That path has unit tests only, and local sign-in with v10 has
