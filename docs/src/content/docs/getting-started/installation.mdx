...
<Aside type="tip">
  `@ic-reactor/react` depends on and re-exports `@ic-reactor/core`, so React
  apps install and import only `@ic-reactor/react` for IC Reactor runtime APIs.
</Aside>
...
[Read more about Vite Plugin](/v4/packages/vite-plugin)
...
[Read more about CLI](/v4/packages/cli)

## Package Overview

| Package                  | Purpose                                 | For React | For Non-React |
| ------------------------ | --------------------------------------- | --------- | ------------- |
| `@ic-reactor/react`      | React hooks + included core runtime     | ✅        | ❌            |
| `@ic-reactor/core`       | Core: Reactor, ClientManager, utilities | included  | ✅            |
| `@ic-reactor/candid`     | Runtime Candid fetching/parsing         | Optional  | Optional      |
| `@ic-reactor/parser`     | WASM `.did` parser outputs              | Optional  | Optional      |
...
1. **Recommended for Vite**: use [`@ic-reactor/vite-plugin`](/v4/packages/vite-plugin)
2. **Recommended for non-Vite or CI**: use [`@ic-reactor/cli`](/v4/packages/cli)
...
Now that you have IC Reactor installed, continue to the [Quick Start](/v4/getting-started/quick-start) guide to build your first integration!
