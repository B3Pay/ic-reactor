...
The [`useAuth`](/v4/reference/createauthhooks/useauth) hook provides login and logout functionality:
...
For accessing the [`useAgentState`](/v4/reference/createauthhooks/useagentstate) connection state:
...
A convenience hook for getting the current user's principal. See [`useUserPrincipal`](/v4/reference/createauthhooks/useuserprincipal) reference:
...
6. **Use Suspense boundaries** — Wrap components using [`useActorSuspenseQuery`](/v4/reference/createactorhooks/useactorsuspensequery) in `<Suspense>`
