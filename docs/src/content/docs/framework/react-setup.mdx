...
- Hooks are client-only, like every React hook — call them from a
  `"use client"` module. A server component cannot import from
  `@ic-reactor/react` at all, not even `Reactor` or `ClientManager`: its entry
  point also loads the hooks, and `next build` fails with "You're importing a
  module that depends on `useSyncExternalStore` into a React Server Component
  module". Import `Reactor`, `DisplayReactor` and `ClientManager` from
