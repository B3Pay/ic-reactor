...
Each unique combination of canister ID, function name, and arguments creates a separate cache entry. The whole `args` array is serialized into a **single** string segment with `JSON.stringify` (BigInt values become decimal strings), so the key is always `[canisterId, functionName, argsJson]` — arguments are never spread into separate elements.
...
the plain `[canisterId, functionName, argsJson]` key.
