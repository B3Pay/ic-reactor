...
A query object and a query factory invalidate their own queries:

```typescript
import { createQueryFactory } from "@ic-reactor/react"
import { backend } from "./reactor"

const getUser = createQueryFactory(backend, { functionName: "getUser" })

function RefreshButton({ userId }: { userId: string }) {
  const handleRefresh = () => {
    // One user's query
    void getUser([userId]).invalidate()
  }

  const handleRefreshAll = () => {
    // Every getUser query this factory made, whatever the args
    void getUser.invalidate()
  }
...
`getUser.getQueryKey()` is the key prefix those queries share, for any other
QueryClient call. For the client itself use `backend.queryClient`: the hooks
bind to the reactor's own client, and `useQueryClient()` throws when there is
no `QueryClientProvider`, which is optional.
...
const { mutate } = useActorMutation({
  ...
  invalidateQueries: [
    { functionName: "getPosts" }, // every getPosts query, whatever its args
    { functionName: "getPostCount" },
  ],
})
```

An entry can also be a query object or a query factory (`[postsQuery]`,
`[getUser]`), or a query key. The invalidation is awaited before `onSuccess`
runs. See [Mutations](/v3/framework/mutations#invalidating-queries-after-mutation).

### Using onSuccess

For more control, use `onSuccess`. Here the new post goes straight into the
cached list instead of being refetched:
...
const { mutate } = useActorMutation({
  functionName: "createPost",
  onSuccess: (newPost) => {
    // `old` is undefined when the list is not cached yet
    postsQuery.setData((old) => (old ? [...old, newPost] : old))
  },
})
```
...
function LikeButton({ postId }: { postId: string }) {
  const { mutate } = useActorMutation({
    ...
    // Cancel the post's fetch in flight, write the new value, keep a rollback
    onMutate: ([postId]) =>
...
### Query Object Cache Controls

Every query object (`createQuery`, `createSuspenseQuery`, the infinite
variants and factory instances) acts on its own cache entry:

| Method                      | What it does                                                                                    |
| --------------------------- | ----------------------------------------------------------------------------------------------- |
| `optimisticUpdate(updater)` | Cancels the fetch in flight, writes `updater(cached)`, resolves with `{ rollback() }`           |
| `cancel()`                  | Cancels the fetch in flight; the entry keeps the value it held before                           |
| `reset()`                   | Clears the entry (or restores `initialData`); a mounted hook refetches, a suspense one suspends |
| `setData(updater)`          | Writes a raw value, or `updater(cached)`, with no cancel and no rollback (not on infinite ones) |
| `invalidate()`              | Marks it stale; a mounted hook refetches                                                        |

`cancel()`, `reset()` and `optimisticUpdate()` match the query's key exactly,
so other args of the same method are left alone. An infinite query's
`optimisticUpdate` gets and returns its `{ pages, pageParams }`.
