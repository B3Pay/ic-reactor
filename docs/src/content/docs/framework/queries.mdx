...
When a query's arguments are not known yet, pass TanStack Query's `skipToken`
in their place. `@ic-reactor/react` re-exports it. The query waits without
calling the canister and fetches as soon as the arguments exist:
...
function UserProfile({ userId }: { userId: string | null }) {
  const { data } = useActorQuery({
    functionName: "getUser",
    // Type-checked as [string] once userId is there: no `!`, no placeholder
    args: userId ? [userId] : skipToken,
  })
}
```

The same goes for a query factory, whose query for `skipToken` has only
`useQuery()`, and for `useActorInfiniteQuery`, as `getArgs`:

```tsx
const getUser = createQueryFactory(backend, { functionName: "getUser" })

const { data } = getUser(userId ? [userId] : skipToken).useQuery()
```

A skipped query has a cache entry of its own under its method's key, which no
call's key shares, so it shows no data until its arguments arrive. Its
`refetch()` has nothing to run (TanStack Query answers it with a "Missing
queryFn" error), so offer a refresh only once the arguments exist. The
suspense hooks and factories do not take `skipToken`, since a suspense query
has to be able to run.

When the arguments are known but the query should still wait, use the
`enabled` option:
