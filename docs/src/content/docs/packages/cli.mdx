...
by that project or by a parent workspace, the CLI formats the `.js` and `.d.ts`
with it and your Prettier config. Regenerating then leaves formatted, committed
declarations unchanged. Without Prettier they hold the Candid parser's output
as-is.
