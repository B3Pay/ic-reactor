...
`ClientManager` exposes provider-neutral helpers for signed OpenID identity
attributes from `@icp-sdk/auth` v6:
...
and pass them to `clientManager.requestIdentityAttributes()`.
