...
A record with exactly one `func` field, such as an `archived_transactions`
entry returned by a ledger's `get_transactions`, resolves as a `funcRecord`
node (`displayType: "func-record"`). Besides `canisterId`, `methodName` and
`funcClass`, it carries `defaultArgs`: the values beside the callback,
arranged as the callback's arguments. When the
...
Create the reactor with `new`, then initialize it:
...
const inputMeta = reactor.getInputMeta("icrc1_transfer")
if (!inputMeta) throw new Error("icrc1_transfer is not in the service")
const { args, defaults, schema } = inputMeta

// Create form with metadata
const form = useForm({
  defaultValues: defaults,
  validators: {
    onBlur: schema  // Zod schema
  },
...
function DynamicFieldInput({ field, fieldApi }) {
  ...
  // Primitive types. nat, int, nat64 and int64 are text fields.
  if (isFieldType(field, "text") || isFieldType(field, "principal")) {
    return (
      <div>
        ...
        />
        {field.format === "timestamp" && field.candidType === "nat64" && (
...
// The resolved result keeps the raw canister value, so you can resolve it
// again yourself with the same metadata
const outputMeta = reactor.getOutputMeta("icrc1_balance_of")
const resolved = outputMeta?.resolve(result.raw)
```
...
<LinkCard
  ...
  href="/v3/packages/candid/candiddisplayreactor"
/>
