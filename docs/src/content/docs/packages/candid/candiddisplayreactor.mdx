...
### Types: with and without a service type

The Candid usually arrives at run time, so a `CandidDisplayReactor` is usually
created without a service type. `A` then defaults to `BaseActor`, and every
method's display-side arguments and results are typed `unknown`: nothing about
their shape is known when the code compiles. Validators receive `unknown` args,
and a `callMethod()` result has to be narrowed before it is rendered or passed
on. (An untyped `CandidReactor` types the same values as `any` instead, because
the raw side passes `BaseActor`'s `any` through.) There are two ways to work
with these values:

- **Cast to the display shape you expect.** For example
  `const [input] = args as [{ to: string; amount: string }]` in a validator, or
  `(await reactor.callMethod(...)) as string` for a `nat` result. The cast is
  not checked against the Candid, so keep it next to the code that relies on it.
- **Pass a service type** when the interface is known at build time. Write it
  in Candid types, as `@icp-sdk/bindgen` generates it; the reactor derives the
  display types from it:
...
const reactor = new CandidDisplayReactor<TokenService>({
  name: "token",
  canisterId: "ryjl3-tyaaa-aaaaa-aaaba-cai",
  clientManager,
  candid: `service : {
...
// input is { to: string; amount: string }: no cast
reactor.registerValidator("transfer", ([input]) =>
  input.to
    ? { success: true }
    : { success: false, issues: [{ path: ["to"], message: "Required" }] }
)
...
The type parameter only names types. The methods the reactor can call still
come from the Candid it parses or fetches, so keep the two in step.
...
| `opt T`          | `T \| undefined` | `[value]` → `value`, `[]` → `undefined`  |
| `blob`           | `string`         | Hex-encoded string (no `0x` prefix)      |
...
Add form validation that works with display types. A reactor created without a
service type passes its validators `unknown` args, so name the display shape
you expect (or pass a service type; see
[Types: with and without a service type](#types-with-and-without-a-service-type)):
...
async function getTokenBalance(canisterId: string, owner: string) {
  ...
  // Pass Principal as string, get balance as string. The untyped reactor
  // returns `unknown`; a nat displays as a string.
  const balance = (await reactor.callMethod({
    functionName: "icrc1_balance_of",
...
// Add form validation (the untyped reactor passes `unknown` args)
reactor.registerValidator("transfer", (args) => {
  const [input] = args as [{ to: string; amount: string }]
...
function useDynamicBalance(reactor: CandidDisplayReactor, owner: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // An untyped reactor returns `unknown`; a nat displays as a string
      return (await reactor.callMethod({
        functionName: "icrc1_balance_of",
