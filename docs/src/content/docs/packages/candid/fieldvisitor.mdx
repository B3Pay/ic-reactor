...
for an empty `opt`, `{ _type }` for a variant. An actor takes Candid values, so
the submitted values are converted to Candid before the call.
...
const form = useForm({
  ...
  onSubmit: async ({ value }) => {
    // Principal, bigint, [] or [x] for an opt, { Tag: payload } for a variant.
    // An untyped codec returns `unknown`, so name the tuple the actor takes.
