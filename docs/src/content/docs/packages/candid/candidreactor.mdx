...
starts with this reactor's interface, Candid source and adapter, so it is ready
to call without another `initialize()`. Its `initialize()` parses the same
source, or fetches its own canister's Candid when there is none. A method
registered on either one afterwards stays on that one. The display and
metadata reactors do the same, and also pass on their validators or build
their metadata for the sibling.
