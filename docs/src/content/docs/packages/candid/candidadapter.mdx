...
try {
  ...
  if (message.includes("Failed to retrieve Candid source")) {
    // Neither the candid:service metadata nor __get_candid_interface_tmp_hack
    console.log("Canister doesn't expose its Candid")
  } else if (message.includes("Failed to compile Candid")) {
