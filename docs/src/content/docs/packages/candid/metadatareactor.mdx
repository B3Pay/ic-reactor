...
// 1) initialize the form with inputMeta.defaults: "" for principals, text
//    and numbers. "" is a valid text, so a text field left empty is sent
//    as ""; principals and numbers fail the schema until the user fills
//    them in. Say the user entered:
const formValues = [
  {
...
## Calling a Func-Record Callback

A record with exactly one `func` field, such as an archived block range in a
ledger's reply, resolves as a `funcRecord` node. Its `defaultArgs` are the
values beside the callback, arranged as the callback's arguments. On
`MetadataReactor` they are Candid values (bigints, a `Principal`, `[]` or `[x]`
for an `opt`), which is what its `callMethod` takes, so a reactor built from
the node's `funcClass` can call the callback with them as they are:

```ts
// `archived` is the resolved funcRecord node, for example
// result.results[0].fields.archived_blocks.items[0]
console.log(archived.defaultArgs) // [{ start: 0n, length: 100n }]
...
const blocks = await archive.callMethod({
  functionName: archived.methodName,
  args: archived.defaultArgs,
})
```

`MetadataDisplayReactor` gives display-typed `defaultArgs` (`"0"` for that
`nat64`) for its own display-typed `callMethod`. Call the callback through the
same kind of reactor that resolved the record.
