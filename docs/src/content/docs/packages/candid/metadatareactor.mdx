...
It extends [`CandidReactor`](/v4/packages/candid/candidreactor) and adds runtime metadata for:
...
Use [`MetadataDisplayReactor`](/v4/packages/candid/metadatadisplayreactor) if you specifically want display-transform behavior for arguments in your app flow.
...
<Steps>
  1. Create and initialize `MetadataReactor`. 2. Read method metadata with
  `getInputMeta` / `getOutputMeta`. 3. Build UI state from `defaults` +
  `schema`. 4. Optionally hydrate from `candidArgsHex` for replay/edit flows. 5.
  Call the method and resolve output through `MethodMeta.resolve`.
</Steps>
...
- [CandidReactor](/v4/packages/candid/candidreactor)
- [CandidFormVisitor](/v4/packages/candid/fieldvisitor)
- [MetadataDisplayReactor](/v4/packages/candid/metadatadisplayreactor)
