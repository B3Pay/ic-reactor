...
If Prettier resolves from `config.root`, the plugin formats the `.js` and
`.d.ts` with it and your Prettier config, so a rebuild leaves formatted,
committed declarations unchanged. Without Prettier they hold the Candid
parser's output as-is, and a formatting error never fails the build.
