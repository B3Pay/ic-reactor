...
When using `dfx` or `icp-cli`, canister IDs are stored in `.icp/cache/mappings/local.ids.json` (or similar). The plugin reads these IDs and injects them into the `ic_env` cookie, mimicking the behavior of asset canisters. This means `ClientManager` can automatically detect the correct canister IDs.
