...
- `index.factories.generated.ts`: managed query and mutation factories, only
  for a canister that sets `factories: true`
- `index.ts`: stable entry wrapper that re-exports from `index.generated.ts`,
  and from `index.factories.generated.ts` when it is generated
...
The file is regenerated with the rest of the canister's bindings when its
`.did` changes. The default `index.ts` wrapper re-exports it; if you have
edited your `index.ts`, the plugin leaves it alone and warns in the terminal
until it re-exports the factories. Setting `factories` back to `false`
removes the file. It needs `target: "react"`. See
...
| `factories`         | `boolean` | Also generate `index.factories.generated.ts`, a query or mutation object per method (default `false`). Needs `target: "react"`. |
...
proxying `/api` to `http://127.0.0.1:4943`, and sets no cookie. With no
canisters configured it still sets one, naming only icp-cli's built-in Internet
Identity at `http://id.ai.localhost:8000/authorize`.
...
`icp` for it; set its `canisterId` and it counts as resolved. If you never run a
local network, set `injectEnvironment: false` and page loads run no `icp`.
...
not origin-isolated, so on a custom domain, on mainnet, and on Codespaces or
Gitpod (whose workspaces share a parent domain with every other user's) the
cookie is ignored.
...
2. regenerates `index.generated.ts`, and `index.factories.generated.ts` for a
   canister that sets `factories: true`
