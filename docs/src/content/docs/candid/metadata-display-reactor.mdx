...
When you call `getArgumentMeta(methodName)`, you receive comprehensive metadata for building input forms:
...
const argMeta = reactor.getArgumentMeta("custom_method")
...
const meta = reactor.getArgumentMeta("my_method") // TypeScript knows the method names
