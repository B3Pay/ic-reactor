...
Authentication hooks are created from an `AuthenticationManager`, which wraps
the shared `ClientManager` without adding authentication APIs to it:
...
<Aside type="note">
  These hooks are bound to the specific `AuthenticationManager` instance you
  created. All actors using the same client manager will share the same
...
IC Reactor can request signed identity attributes from `@icp-sdk/auth` v7 for
...
```tsx
import { IdentityAttributesManager } from "@ic-reactor/react"
...
`identityAttributes.requestOpenId()` directly.
...
- [useAgentState](/v4/reference/createauthhooks/useagentstate) — Agent state reference
- [React Setup](/v4/framework/react-setup) — Complete React configuration
- [Error Handling](/v4/guides/error-handling) — Handle auth and query errors
- [Query Caching](/v4/framework/query-caching) — Understand query invalidation
