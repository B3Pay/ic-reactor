...
<Aside type="note">
  ...
  regardless of the blob's size. (In 3.10 and earlier, blobs longer than 512
  bytes were left as `Uint8Array`; the representation no longer switches with
  payload size. Use the exported `hexToUint8Array` to get bytes back.)
</Aside>
