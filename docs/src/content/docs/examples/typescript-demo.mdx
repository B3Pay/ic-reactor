...
<Aside type="caution">
  `name` is required on every reactor — it is what the `ic_env` cookie lookup
  and the dev-tools logging key off.
</Aside>
...
// Mark every cached query for this canister stale after a write: mounted
// queries refetch, but the entries stay, and fetchQuery (cache-first) still
// returns them until they are refetched
...
// Execute with QueryClient. The wrapper runs the fetch again when a sign-in
// or sign-out overtakes it, instead of returning the previous principal's data
const result = await ledgerReactor.clientManager.fetchAcrossIdentitySwitch(() =>
