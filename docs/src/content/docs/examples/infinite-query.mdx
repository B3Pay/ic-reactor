...
Here `getArgs` spells out its return type. `useReactorInfiniteQuery` receives
the reactor in the same call, so TypeScript types the returned array as
`(bigint | never[])[]` rather than the argument tuple. `as const` doesn't help
for this method: it also makes the empty `opt` array readonly, which
`[] | [string]` rejects.
