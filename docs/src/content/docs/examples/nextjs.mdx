...
<Aside type="caution" title="Why this is a provider and not a module-scope singleton">
  ...
  `AuthenticationManager` is worse: it holds mutable identity state that every
  request would share. The auth hooks' server snapshot is a fixed signed-out
  state, so a server render no longer shows it, but any code that reads
  `authState`, or calls through the shared agent, still gets whichever identity
  the manager holds at that moment.
