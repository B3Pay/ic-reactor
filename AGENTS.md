...
- `@ic-reactor/core` (`packages/core`, `3.12.1`) — core runtime, `ClientManager`, `Reactor`, `DisplayReactor`, cache integration.
- `@ic-reactor/react` (`packages/react`, `3.12.1`) — React bindings, actor hooks, query/mutation factories, Internet Identity auth, and identity-attribute hooks.
- `@ic-reactor/candid` (`packages/candid`, `3.12.1`) — dynamic Candid adapter/reactors and metadata reactors.
- `@ic-reactor/parser` (`packages/parser`, `0.4.8`) — Rust/WASM Candid parser.
...
- Keep package-specific docs aligned with the released `@ic-reactor/core` v3.12.1 runtime and its published tooling.
