...
- `@ic-reactor/codegen` (`packages/codegen`, `0.15.1`) — shared generation pipeline used by CLI and Vite plugin.
- `@ic-reactor/cli` (`packages/cli`, `0.15.1`) — `ic-reactor` CLI for explicit declaration/reactor generation.
- `@ic-reactor/vite-plugin` (`packages/vite-plugin`, `0.15.1`) — Vite plugin for watch-mode generation and local `ic_env` injection.
...
- Snippet check (CI gate; run `pnpm build` first): `pnpm check:snippets` compiles every `ts`/`tsx`/`typescript` fence of `llms.txt`, `llms-full.txt`, `packages/*/llms.txt`, `skill-packages/**/*.md` and the root and package READMEs against the built packages. Fix a failing snippet in its file. The app names snippets take for granted (`./declarations/backend`, `./reactor`, a `clientManager`) live in `scripts/check-snippets/`; never declare a library export there. The consumer guides get no such names: their snippets import everything they use, and `./reactor` there is the guide's own `// src/reactor.ts` snippet. A fence that is not code to paste opts out with ` ```ts nocheck `. See `CONTRIBUTING.md` (Code snippets).
