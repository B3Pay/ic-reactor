...
import SimpleColorSelector from "./ColorSelector"
import { useWatch } from "react-hook-form"
import SimpleLabelEditor from "./LabelEditor"
import { cn } from "../../utils"
import SimpleFieldInput from "./FieldInput"

interface FieldEditorProps extends DraggableProvided {
  name: `items.${number}`
}

const SimpleFieldEditor: React.FC<FieldEditorProps> = ({
  name,
  innerRef,
  dragHandleProps,
  draggableProps,
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        <SimpleLabelEditor name={name} />
        <SimpleFieldInput name={`${name}.value`} />
      </div>
      <div className="w-8 h-8 px-2 text-white hidden sm:block">...</div>
      <SimpleColorSelector name={`${name}.color`} />
    </div>
  )
}
