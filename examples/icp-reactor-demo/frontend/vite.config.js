import { defineConfig } from "vite"
import react from "@vitejs/plugin-react"
import { icReactorAdvancedPlugin } from "@ic-reactor/vite-plugin"
import fs from "fs"
import path from "path"
// ═══════════════════════════════════════════════════════════════════════════
// icp-cli Environment Loader
// ═══════════════════════════════════════════════════════════════════════════
// Automatically reads canister IDs from icp-cli's cache directory.
// In production (deployed to asset canister), these come from the ic_env cookie.
/**
 * Loads canister IDs from icp-cli's local cache
 * Path: .icp/cache/mappings/local.ids.json
 */
function loadCanisterIds() {
  var idsPath = path.resolve(__dirname, "../.icp/cache/mappings/local.ids.json")
  try {
    var content = fs.readFileSync(idsPath, "utf-8")
    return JSON.parse(content)
  } catch (error) {
    console.warn("[icp-cli] Could not load canister IDs from ".concat(idsPath))
    console.warn("[icp-cli] Make sure to run 'icp deploy' first")
    return {}
  }
}
// Load canister IDs from icp-cli cache
var canisterIds = loadCanisterIds()
// Local replica root key (used for development only)
var IC_ROOT_KEY_HEX =
  ""
// Build the ic_env cookie value with all PUBLIC_ canister IDs
function buildIcEnvCookie() {
  var envParts = ["ic_root_key=".concat(IC_ROOT_KEY_HEX)]
  // Add all canister IDs as PUBLIC_CANISTER_ID:<name>
  for (var _i = 0, _a = Object.entries(canisterIds); _i < _a.length; _i++) {
    var _b = _a[_i],
      name_1 = _b[0],
      id = _b[1]
    envParts.push("PUBLIC_CANISTER_ID:".concat(name_1, "=").concat(id))
  }
  return envParts.join("&")
}
console.log("[icp-cli] Loaded canister IDs:", canisterIds)
// https://vite.dev/config/
export default defineConfig({
  plugins: [
    react(),
    // ═══════════════════════════════════════════════════════════════════════
    // IC-Reactor Plugin (Custom!)
    // ═══════════════════════════════════════════════════════════════════════
    // Instead of @icp-sdk/bindgen's default actor generation, this plugin:
    // 1. Generates only the declarations (idlFactory + types)
    // 2. Creates ic-reactor hooks (createQuery, createMutation, etc.)
    // 3. Sets up ClientManager and DisplayReactor automatically
    //
    // Result: Just import and use hooks - zero manual setup!
    icReactorAdvancedPlugin({
      canisters: [
        {
          name: "backend",
          didFile: "../backend/backend.did",
          useDisplayReactor: true,
        },
        {
          name: "counter",
          didFile: "../backend/counter.did",
          useDisplayReactor: true,
        },
      ],
      outDir: "./src/generated",
    }),
  ],
  server: {
    // ═══════════════════════════════════════════════════════════════════════
    // ic_env Cookie (Simulating Asset Canister Behavior)
    // ═══════════════════════════════════════════════════════════════════════
    // In production, the asset canister sets this cookie automatically.
    // For local dev, we simulate it here in the Vite dev server.
    headers: {
      "Set-Cookie": "ic_env=".concat(
        encodeURIComponent(buildIcEnvCookie()),
        "; SameSite=Lax;"
      ),
    },
    // Proxy API calls to local replica (icp-cli uses port 8000)
    proxy: {
      "/api": {
        target: "http://127.0.0.1:8000",
        changeOrigin: true,
      },
    },
  },
})
