...
pub fn program_ir(env: &TypeEnv, actor: &Type, prog: &IDLMergedProg) -> Result<CandidProgramIr> {
    let types = env
        .to_sorted_iter()
        .map(|(name, ty)| {
    ...
    let service = service_ir(env, actor, prog)?;

    Ok(CandidProgramIr { types, service })
}

fn service_ir(env: &TypeEnv, actor: &Type, prog: &IDLMergedProg) -> Result<CandidServiceIr> {
    let service = match env.trace_type(actor)?.as_ref() {
        TypeInner::Class(_, service) => service.clone(),
        TypeInner::Service(_) => actor.clone(),
...
"#,
        ...
        assert_eq!(ir.service.methods[0].mode, "query");
        assert_eq!(ir.service.methods[0].docs, vec!["Balance docs."]);
        assert!(matches!(
            ir.service.methods[0].args[0].typ,
            CandidTypeIr::Ref { .. }
        ));
    }
...
"#,
        ...
        let method = &ir.service.methods[0];
