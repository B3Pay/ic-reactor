...
import { realpathAllowingMissing } from "./paths.js"
...
export function cleanStaleOutput(options: CleanOptions): string[] {
  ...
  for (const entry of fs.readdirSync(globalOutDir, { withFileTypes: true })) {
    ...
    if (keep.directories.has(realpathAllowingMissing(staleDir))) continue
 ...
 * stale.
 *
 * The directories are held as real locations on disk, and the sweep resolves
 * each candidate the same way. Compared as written, an `outDir` that reaches the
 * global `outDir` through a symlink, or that differs from the directory on a
 * case-insensitive volume only in case, named the canister's current output by
 * another path, and the sweep deleted it, `index.ts` included.
 */
function resolveConfiguredOutput(
  ...
  for (const [key, canisterConfig] of Object.entries(config.canisters)) {
    ...
    try {
      ...
      directories.add(realpathAllowingMissing(outDir))
    } catch (error) {
